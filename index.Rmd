...
current_deployments <- 
  current_deployments %>%
    ...
    dplyr::mutate(across(where(is.POSIXct), ~ format(., "%Y-%m-%d")),
                  ...
                  deployment_id = deployment_id %>% factor()
...
receiver_map <- 
  ...
  addPolygons(data = BPNS, color = "darkgrey",
    ...
    addAwesomeMarkers(data = current_deployments,
                   ...
                   popup = ~paste(
                           ...
                           "<b>Battery status:</b> ", ifelse(needs_battery_change %>% is.na(), "Needs manual check",
  ...
  addLegend(position = "topright", opacity = 1,
            colors = current_deployments$colour %>% unique(), labels = c("Needs change", "Battery is OK", "Needs manual check")) %>%
...
flexdashboard::valueBox(num_receivers_manualcheck,
                        ...
                        caption = "currently active deployments <br>need a manual data check in the ETN portal, <br>divided into 3 categories",
...
**Category 3:** For `r current_deployments_3 %>% nrow()` deployments, the `battery_estimated_end_date` is missing or lies in the past.
...
### Table with details about deployments that require manual checking in the ETN portal {data-width=750}
...
DT::datatable(current_deployments %>%
              dplyr::select(deployment_id, receiver_id,, needs_battery_change, station_name, battery_installation_date, battery_estimated_end_date, deploy_date_time, 
                            comments, deploy_latitude, deploy_longitude, acoustic_project_code, mooring_type, previous_deployment_id, previous_battery_installation_date, previous_deploy_date_time,
